...
export default function InsightsPage() {
  const [data, setData] = useState([]);        // Filtered dataset for display
  const [loading, setLoading] = useState(false); 
  const [raw, setRaw] = useState([]);          // Raw dataset from API

  const API_URL = (process.env.REACT_APP_BACKEND_URL || "").replace(/\/$/, "");

  // Compute min & max year in dataset (used for sliders)
  ...
  // Fetch car ownership data from backend on mount
  useEffect(() => {
    let cancelled = false;
    const run = async () => {
      setLoading(true);
  ...
  // Update year range to match dataset once raw data loads
  useEffect(() => {
  ...
  // KPI calculations: latest vehicles, CAGR, peak year, and years covered
  const kpis = useMemo(() => {
  ...
  // Handlers for slider and number inputs
      ...
      <div className="border border-gray-200 rounded-xl p-4 bg-white shadow-sm mt-4">
        ...
        <div className="grid gap-4 grid-cols-1 md:grid-cols-2">
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="fromYear" className="text-xs text-gray-500">From</label>
            <input
              id="fromYear"
              type="range"
              min={minYear}
          ...
          {/* To Year */}
          <div className="flex flex-col gap-1.5">
            <label htmlFor="toYear" className="text-xs text-gray-500">To</label>
            <input
              id="toYear"
              type="range"
              min={minYear}
